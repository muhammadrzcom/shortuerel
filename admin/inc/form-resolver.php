<?php
			...
			if ( isset( $_POST['save'] ) && $auth->checkAuth() ) {
			     ...
			     // update data
			     if ( $id && $action == 'edit' ) {
			          $db->update( $update, $config['table_prefix'] . $config['table_domains_allowed'], 'ID="' . $id . '"' );
			     }
			     if ( $action == 'new' && $host ) {
			          $auth->setGoodLanding( $config['base_url'] . 'admin/resolver.php' );
			          $db->insert( $update, $config['table_prefix'] . $config['table_domains_allowed'] );
			     }
