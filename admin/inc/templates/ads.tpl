        ...
        <div class="logged"><p>Welcome, <span><?php echo $config['username']; ?></span> | <a href="logout.php">Logout</a></p></div>
            ...
            <div id="main">
                <div class="fullwidth">
                        ...
                        <input type="button" class="submit filter" value="add New" onClick="window.location.href='edit.php?action=new&from=ads'" style="margin:4px -20px 0 0;"/>
        				...
        				<table class="data display">
                            ...
                            if ( $urls ){
                                ...
                                foreach($urls as $url ){
                                    ...
                                    $html .= '<td>'.$url['ID'].'</td>';
                                    $html .= '<td><a href="'.$url['url'].'" target="_blank" data-tip="'.$url['url'].'">view</a></td>';
                                    $html .= '<td>'.$url['title'].'</td>';
                                    $html .= '<td>'.$url['views'].'</td>';
                                    $html .= '<td>'.$url['max'].'</td>';
                                    $html .= '<td>'.$url['date'].'</td>';
                                    $html .= '<td><a href="edit.php?action=edit&from=ads&id='.$url['ID'].'">edit</a> | <a href="edit.php?action=delete&from=ads&id='.$url['ID'].'" rel="askdelete">delete</a></td>';
                                    $html .= '</tr>'."\r\n";
                                }
                                echo $html;
                            } 
