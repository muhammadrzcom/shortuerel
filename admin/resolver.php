<?php
            ...
            $auth->setGoodLanding('resolver.php');
            ...
            // total items
            $total_urls = $db->fetch( 'SELECT COUNT(*) AS TOTAL FROM ' . $config['table_prefix'] . $config['table_domains_allowed'] );
            ...
            // the query
            $query = 'SELECT * FROM ' . $config['table_prefix'] . $config['table_domains_allowed'] . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT ' . $pager->getQueryLimit() . ',' . $pager->getQueryOffset();
            $urls = $db->fetch( $query );
            $urls = isset($urls[0]['ID']) ? $urls : false;                        
                  			
            include('inc/templates/resolver.tpl');